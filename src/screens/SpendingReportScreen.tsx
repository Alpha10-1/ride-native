...
import { PAYMENT_METHOD_LABELS } from "../../src/lib/payments";
import {
  StatementPeriod, SpendingTrip,
  getPeriodBounds, shiftPeriod, formatPeriodLabel,
  getRiderSpending, summarizeSpending, exportSpendingReportPdf,
} from "../../src/lib/spendingReport";

export default function SpendingReportScreen() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [period, setPeriod] = useState<StatementPeriod>("weekly");
  const [anchor, setAnchor] = useState(new Date());
  const [trips, setTrips] = useState<SpendingTrip[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [data, profile] = await Promise.all([
        getRiderSpending(start, end),
        getCurrentProfile(),
      ]);
      setTrips(data);
      if (profile) setRiderName(`${profile.first_name} ${profile.last_name}`.trim());
    } catch {
      setTrips([]);
    } finally {
      setLoading(false);
  ...
  const summary = summarizeSpending(trips);
  ...
  return (
    <Screen>
      <RiderHeader subtitle="Spending Report" menuOpen={menuOpen} onMenu={() => setMenuOpen((v) => !v)} />
      <ScrollView
            ...
            <GlassCard style={styles.summaryCard}>
              <View style={styles.summaryRow}>
                ...
                <View style={styles.summaryItem}>
                  ...
                  <Text style={styles.summaryValue}>{formatFare(summary.totalSpentCents)}</Text>
                </View>
              </View>
              <PrimaryButton
                label={exporting ? "Preparing PDF..." : "Download PDF Report"}
                ...
                <GlassCard key={t.trip_id} style={styles.tripRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.tripMeta}>
                      {t.ride_tier} · {PAYMENT_METHOD_LABELS[t.payment_method] ?? t.payment_method}
                    </Text>
                  </View>
